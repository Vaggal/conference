...
exports.run = function (config) {
  ...
  socketio.listen(socketServer, {
    ...
    socket.on('conversation.type.select', function (conversation) {
      ...
      if (totalConversationVotes === (userIds[currentRoom] + 1) && (userIds[currentRoom] + 1) >= 3) {
        if (looseVotes > byTurnVotes) {
          conversations[currentRoom].type = 1;
        } else if (looseVotes < byTurnVotes) {
          conversations[currentRoom].type = 2;
        } else {
          conversations[currentRoom].type = 2; // TODO: We need to find a better solution for this
        }
    ...
    function emitTimeRemaining(room, seconds) {
      room.forEach(function (socket) {
        socket.emit('time.remaining', seconds);
      });
    }

    function findActivePeer() {}

    function talkLoop(room, seconds) {
      emitTimeRemaining(room, seconds);

      talkTimeout = setTimeout(() => {
        console.log(votes.peers);
        activePeer = findActivePeer();
        emitActivePeer(room, peer);
